...
✅ **Липкі заголовки + стовпці списку задач:** заголовок/фільтри сторінок Задачі
(список), Звіти й Проєкти закріплені зверху при прокрутці через обгортку
`components/layout/StickyBar.tsx` (bleed відʼємними полями крізь падінги `main` +
`bg-background`; відʼємний `-mt` виносить sticky-констрейнт до верху вьюпорта).
Список задач тепер таблиця з ширшим набором колонок (проєкт/пріоритет/статус/
виконавець/теги/опис/дати) — конфіг у `features/tasks/taskColumns.ts`
(`useTaskColumns`, localStorage `tht.taskListColumns`), меню видимості
`TaskColumnsMenu` (кнопка «Стовпці», лише у виді «Список»); `TaskList` рендерить
шапку й рядки з одного масиву `COLUMN_DEFS` (гарантія вирівнювання) у
`overflow-x-auto`-контейнері (горизонтальний скрол на вузьких екранах, вертикаль —
через `main`).
