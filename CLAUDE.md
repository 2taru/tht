...
`tht.notifyScope`); UI-гейтинг деструктиву — видалення проєктів і задач лише для
owner/admin (хук `useMyRole`; це ЛИШЕ UI — справжній захист має бути в RLS, поки
`projects`/`tasks` дозволяють усім членам `for all`); онбординг-підказки в таймшиті
...
«Сума» (`lib/money.ts`). ✅ **Імпорт CSV/XLSX** записів часу (`lib/importParse.ts`,
`queries/import.ts`, діалог у Налаштуваннях): автостворення проєктів, пропуск
перетинів. ✅ **Внутрішні сповіщення** (`lib/dueDate.ts`, `NotificationsBell`):
