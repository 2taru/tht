...
- [x] **Перенесення між просторами** (2026-07-01): експорт/імпорт проєктів, задач і
      таймшита у самодостатній JSON (`lib/workspaceTransfer.ts` — посилання за індексом,
      `parseBundle` + тести; `queries/transfer.ts` — резолв проєктів/тегів за назвою,
      створення задач, записи стають записами імпортера з пропуском перетинів).
      UI у Налаштуваннях: `TransferCard` (експорт) + `WorkspaceImportDialog` (імпорт у
      поточний простір). Окремо від CSV-імпорту записів.
- [x] **Командна робота v1.5** (2026-07-02, міграції `0013`–`0016`):
      запрошення незареєстрованих (`workspace_invites` + авто-приєднання в
      `handle_new_user`; `invite_member` → `'added'|'invited'`); коментарі до задач
      (`task_comments`, композитний FK `(task_id, workspace_id)`); сповіщення
      «вам призначено задачу» (`notifications`, пишуться лише тригером; секція
      «Події» у дзвіночку); передача власності (rpc `transfer_ownership`);
      мобільний таймшит — long-press замість `touch-none` (свайп скролить сітку).
- [x] Крос-простірні («загальні») звіти (2026-07-02) — режим «Усі мої простори»:
      лише власні години користувача по всіх просторах (`useReportEntriesAll`),
      розбивка по просторах, колонка «Простір» у CSV.
- [ ] Експорт Excel (xlsx — лише з безпечного джерела/CDN); налаштовувані Kanban-колонки
- [ ] Email-сповіщення / пуші (Edge Function + крон); realtime для команд
